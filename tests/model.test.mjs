...
test("dot summaries apply the strict over-50-percent unknown exclusion", () => {
  ...
  assert.equal(imageDotSummary(mostlyUnknownImage).eligible, false);
  ...
  assert.equal(summary.counted_image_count, 2);
  assert.equal(summary.incomplete_image_count, 0);
});

test("over-threshold completed counts can be replaced after annotation", () => {
  const session = createSession("Gulf");
...
test("CSV round-trip preserves metadata and exact stroke geometry", () => {
  ...
  assert.match(csv, /"session_dot_unknown_threshold_pct"/);
