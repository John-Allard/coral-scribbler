...
| Scope | Columns |
| --- | --- |
| Session | `schema_version`, `session_id`, `dataset_name`, `annotator`, `annotation_mode`, `session_current_image_relative_path`, `session_dot_target_count`, `session_dot_unknown_threshold_pct`, marker diameter/fraction, blink/solid/unlabeled-marker display fields, five `session_hotkey_*` fields, timestamps |
...
the assigned class. If an excluded completed image is re-scattered, the first
sample is replaced rather than exported alongside the new sample; the nonzero
